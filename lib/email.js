...
function composeEmailHtml(payload, ctx) {
  ...
  // قسم الجيو
  const geoSection = `
    <h3 style="margin:16px 0 6px 0;">بيانات الموقع الجغرافي (GeoIP)</h3>
    <table style="border-collapse:collapse;width:100%;max-width:700px;">
      ${row('Country', ctx.geo.country)}
      ${row('Region', ctx.geo.region)}
      ${row('City', ctx.geo.city)}
      ${row('ISP / Org', ctx.geo.org)}
      ${row('Latitude', ctx.geo.latitude)}
      ${row('Longitude', ctx.geo.longitude)}
    </table>
  `;
