...
function composeEmailHtml(payload, ctx) {
  ...
  return `
    <h2 style="margin:0 0 8px 0;color:#1f2937;">زيارة جديدة للموقع</h2>
    ...
    <h3 style="margin:16px 0 6px 0;color:#1f2937;">Payload من الباك إند</h3>
    ${prettyPayload}
  `;
}
...
async function sendVisitEmail({ payload, context }) {
  ...
  // ←←← التعديل المهم هنا: نقسم الإيميلات وننظفها من المسافات
  const recipientEmails = process.env.EMAIL_TO
    .split(',')
    .map(email => email.trim())
    .filter(email => email.length > 0); // لو في سطر فاضي أو مسافات بس

  if (recipientEmails.length === 0) {
    throw new Error('EMAIL_TO is empty or invalid in .env');
  }
  ...
  const mailOptions = {
    ...
    to: recipientEmails, // ←←← nodemailer بيدعم array مباشرة أو string مفصول بفواصل
    subject,
    text: `زيارة جديدة
...
${JSON.stringify(payload, null, 2)}`,
  ...
  console.log(`تم إرسال الإيميل بنجاح إلى ${recipientEmails.length} مستلمين`);
  return info;
}
