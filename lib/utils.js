...
/** تهدئة حسب الـ IP (ذاكرة مؤقتة داخل الدالة السيرفرلس) */
function shouldSendForIp(ip) {
  if (!ip) return true; // لو مش قادر يجيب IP من الأساس، ابعت
...
/** تنظيف IPv6-mapped & قوائم x-forwarded-for */
...
/** استخراج سياق الزيارة (Client/Server/Full URL) */
function getVisitContext(req) {
  ...
  const fullUrl = `${proto}://${host}${(serverPort !== '80' && serverPort !== '443') ? `:${serverPort}` : ''}${path}`;
